...
public class Quaternion {
	...
	public Quaternion nlerp(Quaternion dest, float factor, boolean shortest) {
		...
		return correctDest.sub(this).mul(factor).add(this).normalized();
	}

	public Quaternion slerp(Quaternion dest, float factor, boolean shortest) {
		...
		if(Math.abs(cos) >= 1 - EPSILON)
			return nlerp(correctDest, factor, false);
		float sin = (float)Math.sqrt(1 - cos * cos);
		float angle = (float)Math.atan2(sin, cos);
		float sinInverse = 1 / sin;
		float srcFactor = (float)Math.sin((1 - factor) * angle) * sinInverse;
		float destFactor = (float)Math.sin(factor * angle) * sinInverse;
		return this.mul(srcFactor).add(correctDest.mul(destFactor));
	}
